...
def load_model():
    with open('quikr_car_price_model.pkl', 'rb') as file:
        pipe = pickle.load(file)
    return pipe

def preprocess_and_predict(name, company, year, kms_driven, fuel_type, pipe):
    data = pd.DataFrame([{
        "name": name,
        "company": company,
        "year": year,
...
def main():
    ...
    company = st.selectbox("Car Company", (['Audi', 'BMW', 'Chevrolet', 'Datsun', 'Fiat', 'Force', 'Ford',
    ...
    if st.button("Predict Price"):
        ...
        price = float(predicted_price.squeeze())
        st.success(f"The predicted price of the car is: ₹{price:.2f}")
        conn = get_connection_url()
        cursor = conn.cursor()
        insert_query = """
            INSERT INTO car_price_predictions (name, company, year, kms_driven, fuel_type, predicted_price)
            VALUES (%s, %s, %s, %s, %s, %s)
        """
        cursor.execute(insert_query, (name, company, year, kms_driven, fuel_type, price))
        conn.commit()
        cursor.close()
        conn.close()
